...
def grad_cam(model, image, target_class, layer_name, nb_classes): 
    ...
    target_layer = lambda x: target_category_loss(x, target_class, nb_classes)
    ...
    cam = np.zeros(output.shape[0 : 2], dtype = np.float32)  # New array with the output shape, but with zeros
    ...
    cam = cam if np.max(cam) == 0 else cam /np.max(cam) # There is a fallback if np.max equal to 0
