...
image_name = 'segmented_PAM_with.jpg'
...
layers = layers[9:-8] # Take only convolutional deep layers (That seems to be relevant)
...
print("Class_to_visualize rate: " + str(predictions[0][class_to_visualize]) + ' to Class: (' + class_name[class_to_visualize] + ')')


method_name = ['CAM_IMAGE_JET','CAM_IMAGE_BONE','CAM_AS_WEIGHTS','JUST_CAM_JET','JUST_CAM_BONE']
...
if process == 0: # Visualize specific layer with specific visualization method
    ...
    for layer_to_visualize in layers:
        ...
        save_image(cam_heatmap, './experiments/segmentation/with_segmentation/'+layer_to_visualize+'.png')